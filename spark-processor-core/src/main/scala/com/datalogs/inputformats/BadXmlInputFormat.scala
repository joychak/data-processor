...
private[inputformats] class BadXmlRecordReader extends RecordReader[LongWritable, Text] {
  ...
  private var spillOverTagRead:Boolean = false
  ...
  override def initialize(split: InputSplit, context: TaskAttemptContext): Unit = {
    ...
    val charset =
      Charset.forName(conf.get(BadXmlInputFormat.ENCODING_KEY, "UTF-8"))
    startTag = conf.get(BadXmlInputFormat.START_TAG_KEY).getBytes(charset)
    endTag = conf.get(BadXmlInputFormat.END_TAG_KEY).getBytes(charset)
